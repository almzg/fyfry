...
CGPoint JKRectCenter(CGRect rect)
{
    return CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
}

CGRect JKCenterRect(CGRect rect, CGPoint center)
{
...
static CGFloat const JKScaleFactorAlpha = 0.3;
static CGFloat const JKScaleFactorBeta = 0.4;

CGFloat JKScaledValueForValue(CGFloat value)
{
    return 1.0 / (1.0 + expf(-1 * JKScaleFactorAlpha * powf(value, JKScaleFactorBeta)));
}

@interface JKClusterAnnotationView ()
@property (strong, nonatomic) UILabel *countLabel;
@end

@implementation JKClusterAnnotationView
...
- (void)setCount:(NSUInteger)count
{
    ...
    CGRect newBounds = CGRectMake(0, 0, roundf(44 * JKScaledValueForValue(count)), roundf(44 * JKScaledValueForValue(count)));
    self.frame = JKCenterRect(newBounds, self.center);
    
    CGRect newLabelBounds = CGRectMake(0, 0, newBounds.size.width / 1.3, newBounds.size.height / 1.3);
    self.countLabel.frame = JKCenterRect(newLabelBounds, JKRectCenter(newBounds));
